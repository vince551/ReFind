...
class LocationService : Service() {
    ...
    override fun onCreate() {
        ...
        callback = object : LocationCallback() {
            override fun onLocationResult(result: LocationResult) {
                ...
                Thread {
                    try { ReFindApi.updateLocation(this@LocationService, location.latitude, location.longitude, location.accuracy, null) } catch (_: Exception) { }
                }.start()
            }
        }
    ...
    private fun createChannel() {
        if (Build.VERSION.SDK_INT >= 26) getSystemService(NotificationManager::class.java).createNotificationChannel(NotificationChannel("refind_location", "ReFind recovery protection", NotificationManager.IMPORTANCE_LOW))
    }
